...
WORS_SpyData.debugMode = WORS_SpyData.debugMode or false -- debug prints
WORS_SpyData.autoWildernessMode = WORS_SpyData.autoWildernessMode or true -- true enable all track in wilderness
WORS_SpyData.useKOSList = WORS_SpyData.useKOSList or true -- Set to true to use KOS list, false to track all players
...
--local useKOSList = false  -- Set to true to use KOS list, false to track all players

function initialise()
    print("Initialize called")
    if WORS_SpyData.redTime == nil then
        WORS_SpyData.redTime = 180  -- Set to 180 seconds default 3min
        debugPrint("Initialized redTime: " .. WORS_SpyData.redTime)
    end
    if WORS_SpyData.yellowTime == nil then
        WORS_SpyData.yellowTime = 900  -- Set to 900 seconds default 15min
	...
	if WORS_SpyData.alertMessageDelay == nil then
        WORS_SpyData.alertMessageDelay = 30  -- Set to 30 seconds default
        debugPrint("Initialized alertSoundDelay: " .. WORS_SpyData.alertSoundDelay)
    end		
    ...
    print("autoWildernessMode t/f: " .. tostring(WORS_SpyData.autoWildernessMode))
end
...
local lastAlertSoundTime = 0 -- Tracks the last time a global alert sound was played
local lastRaidWarningTimes = {} -- Tracks the last time a raid warning was shown per player
...
local function OnCombatLogEvent(self, event, ...)
    ...
    if WORS_SpyData.useKOSList then
...
local function HandleKOSCommand(input)
    ...
    if input == "" then
        ...
        if targetName and UnitIsPlayer("target") then
            -- Capitalize the first letter of the target name directly
        ...
        -- Capitalize the first letter of the provided input directly
        input = input:sub(1, 1):upper() .. input:sub(2)
